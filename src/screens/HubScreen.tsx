// HubScreen.tsx – migrated to TypeScript
...
type User = { name?: string; email?: string; avatar?: string; isGuest?: boolean };

type Resume = { id: string; name: string; lastModified: number };
...
const HubScreen: React.FC = () => {
  ...
  const { user, logout, quickStart } = authCtx;
    ...
    <View style={styles.container}>
      <ScrollView style={styles.scrollView} contentContainerStyle={[styles.scrollContent, { paddingBottom: insets.bottom + 20 }]}>
            ...
            <View style={styles.profileSection}>
              ...
              {user && !user.isGuest ? (
                <TouchableOpacity style={styles.logoutBtn} onPress={() => {
                  Alert.alert('Logout', 'Are you sure you want to log out of your profile?', [
                    { text: 'Cancel', style: 'cancel' },
        ...
        <View style={styles.menuContainer}>
          ...
          <AppCard title="Resume Builder" description="Professional templates & South African context features." icon="file-document-edit" color="#6366f1" onPress={async () => { if (!user) await quickStart(); navigation.navigate('ResumeHome'); }} />
...
const styles = StyleSheet.create({
  ...
  scrollView: { flex: 1 },
  header: { paddingBottom: 30, borderBottomLeftRadius: 30, borderBottomRightRadius: 30, paddingHorizontal: 24 },
  ...
  subtitleText: { color: 'rgba(255,255,255,0.8)', marginTop: 4 },
  dashboardSubtitle: { color: 'rgba(255,255,255,0.6)', marginTop: 2 },
  headerLeft: { flex: 1 },
  profileSection: { alignItems: 'center', justifyContent: 'center' },
  avatarFallback: { backgroundColor: '#6366f1' },
  logoutBtn: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255,50,50,0.15)', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12, marginTop: 8, borderWidth: 1, borderColor: 'rgba(255,50,50,0.3)' },
  logoutText: { color: '#ff6b6b', fontSize: 10, fontWeight: 'bold', marginLeft: 4, textTransform: 'uppercase' },
  loginBtn: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(99,102,241,0.15)', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12, marginTop: 8, borderWidth: 1, borderColor: 'rgba(99,102,241,0.3)' },
  loginText: { color: '#818cf8', fontSize: 10, fontWeight: 'bold', marginLeft: 4, textTransform: 'uppercase' },
  menuContainer: { marginTop: 20 },
  scrollContent: { paddingBottom: 40 },
  sectionTitle: { fontWeight: 'bold', marginBottom: 16, color: '#333', paddingHorizontal: 24 },
  cardContainer: { marginBottom: 16, paddingHorizontal: 24 },
  appCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', borderRadius: 16, padding: 12, borderWidth: 1, borderColor: '#f0f0f0' },
  iconContainer: { width: 60, height: 60, borderRadius: 12, justifyContent: 'center', alignItems: 'center' },
  cardText: { flex: 1, marginLeft: 16, marginRight: 8 },
