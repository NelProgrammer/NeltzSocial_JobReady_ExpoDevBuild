...
const LoginScreen: React.FC<LoginScreenProps> = () => {
  ...
  const { user, profiles, login, createProfile, deleteProfile, quickStart } = useContext(AuthContext) as any;
        ...
        <ScrollView contentContainerStyle={[styles.scrollContent, { paddingTop: insets.top + 12, paddingBottom: Math.max(insets.bottom, 20) }]}>
          ...
          <Surface style={styles.surface} elevation={0}>
            ...
            <View style={styles.socialGrid}>
              <Button mode="contained" icon="google" onPress={() => {}} style={[styles.socialBtn, { backgroundColor: '#4285F4' }]} labelStyle={{ color: '#fff' }}>
                Google
              </Button>
              <Button mode="contained" icon="linkedin" onPress={() => {}} style={[styles.socialBtn, { backgroundColor: '#0077B5' }]} labelStyle={{ color: '#fff' }}>
                LinkedIn
              </Button>
            </View>
            <View style={[styles.socialGrid, { marginTop: 12 }]}>
              <Button mode="contained" icon="facebook" onPress={() => {}} style={[styles.socialBtn, { backgroundColor: '#1877F2' }]} labelStyle={{ color: '#fff' }}>
                Facebook
              </Button>
              <Button mode="contained" icon="twitter" onPress={() => {}} style={[styles.socialBtn, { backgroundColor: '#000' }]} labelStyle={{ color: '#fff' }}>
                Twitter
              </Button>
            </View>
              ...
              {[...profiles].sort((a, b) => (b.lastLogin || 0) - (a.lastLogin || 0)).slice(0, 5).map((p: Profile) => (
                <View key={p.id} style={styles.profileItem}>
                  <TouchableOpacity style={styles.profileInfo} onPress={() => login(p.id)}>
                    <Avatar.Icon size={32} icon="account" style={{ backgroundColor: "#6366f1" }} />
                    <View style={{ marginLeft: 12 }}>
                      <Text style={styles.profileName}>{p.name}</Text>
                      <Text style={styles.profileSub}>{p.socialLinks?.google ? 'Social Account' : 'Local Profile'}</Text>
                    </View>
                  </TouchableOpacity>
                  <IconButton icon="trash-can-outline" iconColor="rgba(255,0,0,0.4)" size={20} onPress={() => deleteProfile(p.id)} />
                </View>
              ))}
            </View>
